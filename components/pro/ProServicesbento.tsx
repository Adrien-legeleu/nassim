'use client';

import { motion } from 'framer-motion';
import {
  PanelsTopLeft,
  Building2,
  Sparkles,
  Droplets,
  ShieldCheck,
} from 'lucide-react';
import { BentoGridShowcaseProAuto } from '../ui/bento-service-features-auto-pro';

export default function ProServicesBento() {
  return (
    <section className="relative bg-white text-neutral-900 w-full overflow-hidden py-32 px-5">
      {/* Halo + ruban tricolore subtil */}
      <div aria-hidden className="pointer-events-none absolute inset-0 z-0">
        <div
          className="absolute left-1/2 top-[45%] -translate-x-1/2 -translate-y-1/2 w-[1000px] h-[1000px] rounded-full blur-[140px] opacity-[0.18]"
          style={{
            background:
              'radial-gradient(circle at 50% 50%, rgba(30,41,59,0.2) 0%, rgba(255,255,255,0) 70%)',
          }}
        />
        <div
          className="absolute rotate-[12deg] left-[10%] top-[35%] w-[1200px] h-[60px] opacity-[0.25] blur-[40px]"
          style={{
            background:
              'linear-gradient(90deg, #0052b4 0%, #ffffff 50%, #d80027 100%)',
          }}
        />
        <div className="hidden md:block absolute top-10 right-1/3 translate-x-1/2 opacity-[0.04]">
          <Droplets className="w-64 h-64 text-slate-900" />
        </div>
        {/* cadre intérieur */}
        <div className="absolute inset-[10%] rounded-[2.1rem] md:rounded-[2.6rem] border border-dashed border-neutral-200/70" />
      </div>

      <div className="relative z-10 mx-auto w-full max-w-[1200px]">
        {/* Heading */}
        <div className="text-center max-w-xl mx-auto mb-16">
          <motion.div
            initial={{ opacity: 0, y: 8 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5, ease: 'easeOut' }}
            className="inline-flex items-center gap-2 rounded-full px-3 py-1 text-[11px] font-medium tracking-wide bg-neutral-100 text-neutral-600"
          >
            Sans survente · Méthode complète
          </motion.div>

          <motion.h2
            initial={{ opacity: 0, y: 8 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.4 }}
            transition={{ duration: 0.6, ease: 'easeOut' }}
            className="mt-4 text-2xl md:text-4xl font-semibold tracking-tight tricolor-underline pb-2"
          >
            Prestations nettes — adaptées aux pros
          </motion.h2>

          <motion.p
            initial={{ opacity: 0, y: 8 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, ease: 'easeOut', delay: 0.05 }}
            className="text-sm md:text-base text-neutral-600 leading-relaxed mt-4"
          >
            Interventions tôt le matin, entre midi et deux, ou après fermeture.
            Résultat immédiatement présentable.
          </motion.p>
        </div>

        {/* Grid bento (même composant que l'Auto) */}
        <div className="relative rounded-[2.5rem] md:rounded-[3rem] border border-neutral-100 bg-white/70 shadow-[0_40px_120px_-30px_rgba(15,23,42,0.1)] p-5 md:p-7 backdrop-blur-[2px]">
          <BentoGridShowcaseProAuto
            className="relative z-10 mx-auto max-w-[1200px]"
            cardClassName="
              rounded-[2rem] h-full p-6 md:p-7
              border border-neutral-200/60
              shadow-[0_25px_80px_-30px_rgba(15,23,42,0.3)]
              bg-white/85
              backdrop-blur-[3px]
              transition-transform duration-300 hover:scale-[1.015]
            "
            integration={
              <FeatureCard
                icon={<PanelsTopLeft className="h-5 w-5 text-blue-700" />}
                title="Vitrines & façades commerciales"
                items={[
                  'Vitres sans traces, joints & encadrements',
                  'Décollement d’adhésifs vitrines',
                  'Avant ouverture / après fermeture',
                  'Accès sécurisé, périmètre propre',
                ]}
                color="blue"
                footer="Focus transparence et angle d’incidence (pas d’auréoles)."
              />
            }
            trackers={
              <FeatureCard
                icon={<Building2 className="h-5 w-5 text-neutral-700" />}
                title="Bureaux & accueils"
                items={[
                  'Plans de travail / écrans',
                  'Points de contact (poignées, interrupteurs)',
                  'Aspiration & entretien moquettes',
                  'Sanitaires : prêt à l’usage',
                ]}
                color="neutral"
              />
            }
            statistic={
              <FeatureCard
                icon={<Sparkles className="h-5 w-5 text-neutral-700" />}
                title="Après-chantier léger"
                items={[
                  'Évacuation poussières fines',
                  'Vitres & encadrements après pose',
                  'Sols protégés puis remis au propre',
                ]}
                color="neutral"
              />
            }
            shortcuts={
              <FeatureCard
                icon={<Droplets className="h-5 w-5 text-red-700" />}
                title="Copropriétés & villas"
                items={[
                  'Cages d’escalier / paliers / ascenseurs',
                  'Baies vitrées & garde-corps',
                  'Terrasses, seuils & détails visibles',
                  'Gestion des passages récurrents',
                ]}
                color="red"
              />
            }
            integrationClassName="bg-blue-50/40 border-blue-100/50"
            trackersClassName="bg-neutral-50/40 border-neutral-100/50"
            statisticClassName="bg-neutral-50/40 border-neutral-100/50"
            shortcutsClassName="bg-red-50/40 border-red-100/50"
          />
        </div>
      </div>
    </section>
  );
}

/* === Sous-composant : FeatureCard (identique Auto, wording PRO) === */
function FeatureCard({
  icon,
  title,
  items,
  color,
  footer,
}: {
  icon: React.ReactNode;
  title: string;
  items: string[];
  color?: 'blue' | 'red' | 'neutral';
  footer?: string;
}) {
  const colorMap = {
    blue: {
      badge: 'bg-blue-100 border-blue-100 text-blue-700',
      bullet: '#2563eb',
    },
    red: {
      badge: 'bg-[#ed293925] border-[#ed293925] text-red-700',
      bullet: '#dc2626',
    },
    neutral: {
      badge: 'bg-neutral-100 border-neutral-100 text-neutral-700',
      bullet: '#6b7280',
    },
  };
  const tone = colorMap[color ?? 'neutral'];

  return (
    <div className="flex flex-col justify-between h-full">
      <div>
        <div className="flex items-start gap-3">
          <div
            className={`flex h-10 w-10 items-center justify-center rounded-2xl border font-medium ${tone.badge}`}
          >
            {icon}
          </div>
          <div className="font-medium leading-tight text-base md:text-lg">
            {title}
          </div>
        </div>

        <ul className="mt-5 flex flex-col gap-2 text-neutral-700 text-sm leading-relaxed">
          {items.map((l, i) => (
            <li key={i} className="flex gap-2">
              <div
                className="mt-[7px] h-[6px] w-[6px] rounded-full"
                style={{ background: tone.bullet }}
              />
              <span>{l}</span>
            </li>
          ))}
        </ul>

        {footer && (
          <div className="mt-5 text-neutral-500 text-xs leading-relaxed">
            {footer}
          </div>
        )}
        {title === 'Vitrines & façades commerciales' && (
          <div className="mt-5 bg-blue-50 border border-blue-200/40 rounded-xl p-3 flex items-start gap-3 text-xs text-neutral-700 shadow-[inset_0_1px_6px_rgba(255,255,255,0.8)]">
            <div className="flex h-6 w-6 items-center justify-center rounded-lg bg-blue-100 border border-blue-100 text-blue-600">
              <ShieldCheck className="w-3.5 h-3.5" />
            </div>
            <div className="leading-snug">
              <strong className="font-medium">Fréquence moyenne :</strong> 1×
              par semaine ou 1× toutes les 2 semaines, selon trafic et saison.
              <br />
              <span className="text-[11px] text-neutral-500">
                Créneaux tôt le matin / tard le soir, passages ajustables en cas
                de pics d’activité.
              </span>
            </div>
          </div>
        )}
      </div>
    </div>
  );
}
